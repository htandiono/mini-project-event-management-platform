...
export const transactionsApi = {
  listTransactions: (params?: { status?: string; eventId?: string }) =>
    fetchApi<OrganizerTransactionItem[]>("/organizer/transactions", { method: "GET", params }),

  acceptProof: (id: string) =>
    fetchApi<OrganizerTransactionItem>(`/organizer/transactions/${id}/accept`, {
      method: "PATCH",
    }),

  rejectProof: (id: string, data: RejectTransactionDTO) =>
    fetchApi<OrganizerTransactionItem>(`/organizer/transactions/${id}/reject`, {
      method: "PATCH",
      data,
    }),

  markAttendance: (id: string) =>
    fetchApi<OrganizerTransactionItem>(`/organizer/transactions/${id}/attend`, {
      method: "PATCH",
    }),
};
